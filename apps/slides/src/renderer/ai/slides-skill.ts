import type { AgentSkill, ToolDisplay } from '@prismoffice/agent-core'
import type {
  GroupRenderNode,
  PictureRenderNode,
  RenderNode,
...
const AGENT_SYSTEM_PROMPT = `You are the AI assistant inside PrismOffice Slides (a slide editor), helping users improve and generate presentations.
...
function _countSpecificFigures(text: string): number {
...
function _imageFailNote(fails?: { page: number; url: string }[]): string {
...
async function executeTool(
  ...
  switch (call.name) {
    ...
    default:
      return fail(t('aiErrUnknown'), `Unknown tool: ${call.name}`)
  }
}
